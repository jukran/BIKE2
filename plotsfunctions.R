...
distPlot2_1 <- function(input_lim, food_consum, unit_consum, input_upper, input_lower, n_sim, input_selectdist, input_selectscale, foodnamesused, nfused, foodindex,
  ...
  # legend ----
  # outside the figure, but onto the current plot, so it is part of the png file:
  mtext(paste("Estimated consumption frequency for", foodnamesused[i], ": ", round(quantile(100*ppred[,foodindex[i]],0.5,names=FALSE),2),
              "% (posterior median)."),
        side = 1, adj = 0,line=1, cex = 1,
        outer = TRUE)
  mtext(paste("95% uncertainty interval for the consumption frequency: ", 
              round(quantile(100*ppred[,foodindex[i]],0.025,names=FALSE),2),"%-", round(quantile(100*ppred[,foodindex[i]],0.975,names=FALSE),2),"%"),
        side = 1, adj = 0,line=2, cex = 1,
        outer = TRUE)
    ...
    for(h in 1:nhusedK){
      ...
      for(i in 1:nfused){
        if(nexactK[hazardindexK[h],foodindex[i]]>0){ # this hazard-food is modeled
          ...
          if(input_selectdist=="Cumulative"){
            ...
            if(input_selectscale=="Absolute"){
              ...
              for(resample in 1:40){   
                ...
                # create 20000 simulations from each replicated data:
                sampleOIM <- sample(sampleOIM,20000,replace=TRUE)
                samplecon <- sample(samplecon,20000,replace=TRUE)
                samplecon0 <- sample(samplecon0,20000,replace=TRUE)
                lines(ecdf(sampleOIM*mean(samplecon)*RK[foodindex[i],hazardindexK[h]]),verticals=TRUE,do.points=FALSE,lwd=1,lty=3,col="#D0006F")
                lines(ecdf(sampleOIM*mean(samplecon0)*RK[foodindex[i],hazardindexK[h]]),verticals=TRUE,do.points=FALSE,lwd=1,lty=3,col="#004F71")
              }
              lines(meanexposure,cump,col="#F7CE3C",main=paste(hazardnamesusedK[h],"from",foodnamesused[i],"(chronic)"),xlab="C.exposure/bw+",ylab="",xlim=c(0,maxx),lwd=3,type="l") 
            ...
            if(input_selectscale=="Logarithmic"){
              ...
              for(resample in 1:40){
                ...
                samplecon <- sample(samplecon,20000,replace=TRUE)
                samplecon0 <- sample(samplecon0,20000,replace=TRUE)
                lines(ecdf(log(sampleOIM*mean(samplecon)*RK[foodindex[i],hazardindexK[h]])/log(10)),verticals=TRUE,do.points=FALSE,lwd=1,lty=3,col="#D0006F")
                lines(ecdf(log(sampleOIM*mean(samplecon0)*RK[foodindex[i],hazardindexK[h]])/log(10)),verticals=TRUE,do.points=FALSE,lwd=1,lty=3,col="#004F71")
              }
              lines(logmeanexposure/log(10),cump,main=paste(hazardnamesusedK[h],"from",foodnamesused[i],"(chronic)"),xlab="log (C.exposure/bw+)",ylab="",xlim=c(minn/log(10),maxx/log(10)),lwd=3,type="l") 
          ...
          mtext(paste("Estimated frequency of exposure for", hazardnamesusedK[h],"from",foodnamesused[i], ": ",round(quantile(100*PK[foodindex[i],hazardindexK[h]]*pK[,hazardindexK[h],foodindex[i]]*ppred[,foodindex[i]],0.5,names=FALSE),2),
                      "% (posterior median)"),
                side = 1, adj = 0,line=0, cex = 1,
                outer = TRUE)
          mtext(paste("95% uncertainty interval for the frequency of exposure: ", 
                      round(quantile(100*PK[foodindex[i],hazardindexK[h]]*pK[,hazardindexK[h],foodindex[i]]*ppred[,foodindex[i]],0.025,names=FALSE),2),"% -", 
                      round(quantile(100*PK[foodindex[i],hazardindexK[h]]*pK[,hazardindexK[h],foodindex[i]]*ppred[,foodindex[i]],0.975,names=FALSE),2),"%"),
                side = 1, adj = 0,line=1, cex = 1,
                outer = TRUE)
    ...
    for(h in 1:nhusedM){
      ...
      for(i in 1:nfused){
        if(nexactM[hazardindexM[h],foodindex[i]]>0){ # this hazard-food is modeled
          ...
          if(input_selectdist=="Density"){
            ...
            if(input_selectscale=="Absolute"){   
              ...
              for(u in 1:n_sim){ 
                ...
                if(input_modelchoice == "Dependent days"){
                  pconsume[1:V] <- rep(ppred[u,foodindex[i]],V) 
                } 
          ...
          if(input_selectdist=="Cumulative"){
            ...
            if(input_selectscale=="Absolute"){
              ...
              for(resample in 1:40){
                ...
                samplecon0 <- sample(concentrations0,length(concentrations0),replace=TRUE)
                # create 20000 simulations from each replicated data:
                sampleser <- sample(sampleser,20000,replace=TRUE)
                samplecon <- sample(samplecon,20000,replace=TRUE)
                samplecon0 <- sample(samplecon0,20000,replace=TRUE)
                lines(ecdf(sampleser*samplecon*RM[foodindex[i],hazardindexM[h]]),verticals=TRUE,do.points=FALSE,lwd=1,lty=3,col="#D0006F")
                lines(ecdf(sampleser*samplecon0*RM[foodindex[i],hazardindexM[h]]),verticals=TRUE,do.points=FALSE,lwd=1,lty=3,col="#004F71")
              }
              lines(meanexposure,cump,col="#F7CE3C",main=paste(hazardnamesusedM[h],"from",foodnamesused[i],"(acute)"),xlab="A.exposure+",ylab="",xlim=c(0,maxx),type="l",lwd=3) 
              lines(medianexposure,cump,lwd=3)
            ...
            if(input_selectscale=="Logarithmic"){
              ...
              for(resample in 1:40){
                ...
                samplecon <- sample(samplecon,20000,replace=TRUE)
                samplecon0 <- sample(samplecon0,20000,replace=TRUE)
                lines(ecdf(log(sampleser*samplecon*RM[foodindex[i],hazardindexM[h]])/log(10)),verticals=TRUE,do.points=FALSE,lwd=1,lty=3,col="#D0006F")
                lines(ecdf(log(sampleser*samplecon0*RM[foodindex[i],hazardindexM[h]])/log(10)),verticals=TRUE,do.points=FALSE,lwd=1,lty=3,col="#004F71")
              }
              lines(logmeanexposure/log(10),cump,main=paste(hazardnamesusedM[h],"from",foodnamesused[i],"(acute)"),xlab="log (A.exposure+)",ylab="",xlim=c(minn/log(10),maxx/log(10)),lwd=3,type="l") 
            } # end of if logarithmic
            #legend("bottomright",cex=0.6,paste(c("expo (2.5 %):","expo (50 %):","expo (97.5 %):"),round(quantile(100*PM[foodindex[i],hazardindexM[h]]*pM[,hazardindexM[h],foodindex[i]]*ppred[,foodindex[i]],c(0.025,0.5,0.975),names=FALSE),2),c("%","%","%")  ))
          ...
          mtext(paste("Estimated frequency of exposure for", hazardnamesusedM[h],"from",foodnamesused[i], ": ",round(quantile(100*PM[foodindex[i],hazardindexM[h]]*pM[,hazardindexM[h],foodindex[i]]*ppred[,foodindex[i]],0.5,names=FALSE),2),
                      "% (posterior median)"),
                side = 1, adj = 0,line=0, cex = 1,
                outer = TRUE)
          mtext(paste("95% uncertainty interval for the frequency of exposure: ", 
                      round(quantile(100*PM[foodindex[i],hazardindexM[h]]*pM[,hazardindexM[h],foodindex[i]]*ppred[,foodindex[i]],0.025,names=FALSE),2),"%-", 
                      round(quantile(100*PM[foodindex[i],hazardindexM[h]]*pM[,hazardindexM[h],foodindex[i]]*ppred[,foodindex[i]],0.975,names=FALSE),2),"%"),
                side = 1, adj = 0,line=1, cex = 1,
                outer = TRUE) 
  ...
  if (!is.element("None", input_selectQ)) {
          ...
          for(v in 1:nV){ # for nV variable values per each parameter set
            ...
            if(nf>1){ # if many foods
              if(input_modelchoice2=="Yes" ){ # variability between users' frequencies
                logitpmc[v,1:nf] <- rmvnorm(1,logitp0[mc[u],1:nf],Sp[mc[u],1:nf,1:nf])
              }
              ...
              musmc[v,1:nf] <- rmvnorm(1,mus0[mc[u],1:nf],Ss0[mc[u],1:nf,1:nf]) # individual mean amount
              
            }
            
            if(nf==1){ # if only one food
              if(input_modelchoice2=="Yes" ){ # variability between users' frequencies
                logitpmc[v,1] <- rnorm(1,logitp0[mc[u],1],sqrt(Sp[mc[u],1,1]))
              }
              if(input_modelchoice2=="No" ){ # no variability between users' frequencies  
                logitpmc[v,1] <- logitp0[mc[u],1]
              }
              pmc[v,1] <- exp(logitpmc[v,1])/(1+exp(logitpmc[v,1])) # individual use probability
              musmc[v,1] <- rnorm(1,mus0[mc[u],1],sqrt(Ss0[mc[u],1,1])) # individual mean amount
            }
        ...
        for(h in 1:nhusedK){
          ...
          if(input_selectscale=="Logarithmic"){
            ...
            plot(ecdf(log(exposurevarsample[1,1:nV])/log(10)),verticals=TRUE,do.points=FALSE,yaxt="s",
                 ...
                 main=paste("Exposure:",hazardnamesusedK[h],"total from",nftotK,"foods (chronic)"))
            for(a in 2:thin){
              lines(ecdf(log(exposurevarsample[a,1:nV])/log(10)),verticals=TRUE,do.points=FALSE,lwd=1,lty=3,col="#D0006F")     
            }
    ...
    if((nhusedM>0)&(nfused>0)){  
      ...
      if(input_modelchoice == "Independent days"){
        ...
        # 2D simulation of uncertainty & variability:
        for(u in 1:nU){ # for nU parameter sets
          wmc[1:nV] <- rlnorm(nV,muw[u],sigw[u]) # bodyweight for v:th individual
          for(v in 1:nV){ # for nV variable values per each parameter set
            if(nf>1){ # if many foods
              if(input_modelchoice2=="Yes" ){ # variability between users' frequencies
                logitpmc[v,1:nf] <- rmvnorm(1,logitp0[mc[u],1:nf],Sp[mc[u],1:nf,1:nf])
              ...
              musmc[v,1:nf] <- rmvnorm(1,mus0[mc[u],1:nf],Ss0[mc[u],1:nf,1:nf])
            ...
            if(nf==1){ # if only one food
              ...
              musmc[v,1] <- rnorm(1,mus0[mc[u],1],sqrt(Ss0[mc[u],1,1]))
              smc[v,1] <- rlnorm(1,musmc[v,1],sqrt(Ss[mc[u],1,1]))  # actual amount 
            }
            for(h in 1:nhM){
              # actual contamination yes/no:
              Imc[v,h,1:nf] <- rbinom(nf,rep(1,nf),pM[mc[u] ,h,1:nf]*PM[1:nf,h]) 
              # actual concentration level:
              cmc[v,h,1:nf] <- rlnorm(nf,mucM[mc[u],h,1:nf],sigcM[mc[u],h,1:nf]) 
            } 
            for(h in 1:nhusedM){
              for(i in 1:nfused){
                if(nexactM[hazardindexM[h],foodindex[i]]==0){ # hazard-food not modeled
                  poissonmeansall[v,h,i] <- 0
                  poissonmeanspos[v,h,i] <- 0
                }
                if(nexactM[hazardindexM[h],foodindex[i]]>0){ 
                  # (poisson)mean exposure for day serving when contaminated & consumed:
                  poissonmeanspos[v,h,i] <- smc[v,foodindex[i]]*
                    RM[foodindex[i],hazardindexM[h]]*
                    cmc[v,hazardindexM[h],foodindex[i]]*wmc[v]
                  # (poisson)mean exposure for any day incl. zeros:
                  poissonmeansall[v,h,i] <- Imc[v,hazardindexM[h],foodindex[i]]*
                    Umc[v,foodindex[i]]*
                    smc[v,foodindex[i]]*
                    RM[foodindex[i],hazardindexM[h]]*
                    cmc[v,hazardindexM[h],foodindex[i]]*wmc[v]
              ...
              if(sum(poissonmeansall[v,h,1:nfused])<=10000){
              ...
              if(sum(poissonmeanspos[v,h,1:nfused])<=10000){
        ...
        for(h in 1:nhusedM){
          ...
          if(input_selectscale=="Absolute"){
            ...
            for(a in 2:thin){
              if(sum(!is.na(exposurevarsample[a,]))>0){   
                lines(ecdf(exposurevarsample[a,!is.na(exposurevarsample[a,])]),verticals=TRUE,do.points=FALSE,lwd=1,lty=3,col="#D0006F")     
              }
            }
            ...
            # legend outside the figure, but onto the current plot, so it is part of the png file:
            mtext(paste("Estimated",theQ,"% quantile for the positive exposures: ",round(quantile(Qplus[,h],0.5,names=FALSE),2),  
                        "(median). 90% uncertainty interval for the estimated quantile is", round(quantile(Qplus[,h],0.05,names=FALSE),2),"-", round(quantile(Qplus[,h],0.95,names=FALSE),2)),
                  side = 1, adj = 0,line=1, cex = 1,
                  outer = TRUE)
          ...
          if(input_selectscale=="Logarithmic"){
            ...
            for(a in 2:thin){
              if(sum(!is.na(exposurevarsample[a,]))>0){   
                lines(ecdf(log10(exposurevarsample[a,!is.na(exposurevarsample[a,])])),verticals=TRUE,do.points=FALSE,lwd=1,lty=3,col="#D0006F")     
              }
            }
      ...
      if(input_modelchoice == "Dependent days"){
        ...
        for(u in 1:nU){ # for nU parameter sets
          for(v in 1:nV){ # for nV variable values per each parameter set
            wmc[v] <- rlnorm(1,muw[u],sigw[u]) # bodyweight
            ...
            for(h in 1:nhM){
              Imc[v,h,1:nf] <- rbinom(nf,rep(1,nf),pM[mc[u] ,h,1:nf]*PM[1:nf,h]) # actual contamination yes/no
